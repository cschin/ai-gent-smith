...
use agent_workspace::*;
use ai_gent_lib::llm_agent::{LlmFsmAgentConfig, LlmFsmAgentConfigBuilder, StatePrompts};
...
fn show_basic_agent_setting(
    ...
    let fsm_config = LlmFsmAgentConfigBuilder::from_toml(&fsm_agent_config)
        .unwrap_or_default()
        .build()
        .unwrap_or_default();
...
fn get_basic_fsm_agent_config_toml_string(sys_prompt: String, follow_up: Option<String>) -> String {
    ...
    .render()
    .unwrap();
    let c = LlmFsmAgentConfigBuilder::from_toml(&simple_agent_config)
        .unwrap()
        .build()
        .unwrap();
